...
class PubmedSearcher(object):
    ...
    xpath_year = ['MedlineCitation', 'Article', 'Journal',
                  'JournalIssue', 'PubDate', 'Year']
    xpath_pub_month = ['MedlineCitation', 'Article', 'Journal',
                       'JournalIssue', 'PubDate', 'Month']
    xpath_pub_day = ['MedlineCitation', 'Article', 'Journal',
                     'JournalIssue', 'PubDate', 'Day']
    xpath_vol = ['MedlineCitation', 'Article', 'Journal',
                 'JournalIssue', 'Volume']
    ...
    extract_date_pubmed_created = extract_xpath_factory(
        'date_pubmed_created', fmt=format_ddate)
    extract_date_pubmed_updated = extract_xpath_factory(
        'date_pubmed_updated', fmt=format_ddate)
    ...
    def extract_grantlist(self, xml_data):
        try:
            ...
            for node in self.xpath_grantlist:
                data = data[node]
            fields = {'acronym': 'Acronym', 'agency': 'Agency',
                      'country': 'Country', 'number': 'GrantID'}
    ...
    def fetch_field_factory(field):
        ...
        def fetch_field(self, pmid):
            targets = [getattr(self, 'extract_' + field)(
                parsed) for parsed in self.fetch_xml(pmid)]
...
obj = PubmedSearcher(email='wjuma@students.usiu.ac.ke')
obj.fetch_all('1')
